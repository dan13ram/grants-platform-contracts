...
/* Simplifications
- no manager for grant whatsoever
  - if we want to update, we deploy a new grant contract
  - no refunds -> if you lose keys, tell streaming funders to deploy a new contract
  - don't be a dinkus use a multisig
- pure pass-through payment splitter 
  - ETH-only
- anyone points a sablier stream to this contract
  - periodically, cron job / admin "harvests" ETH allocation to grant stream
  - anyone can call this function

Questions:
- can the split be done simply by sending ETH -> can we use the fallback function
 ...
 */
contract St is AbstractGrant, ReentrancyGuard {
